...
class MenuViewController: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    public func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if(indexPath.row == 0)
        {
            var kNavigationBarY = 0;
            if #available(iOS 11.0 , *)
            {
                kNavigationBarY = 20;
            }
            let dayEventVC = storyBoard.instantiateViewController(withIdentifier: "ScheduledEvents")
            self.navigationController?.present(dayEventVC, animated: true)
            let navBar : UINavigationBar = UINavigationBar.init(frame: CGRect(x: 0, y:kNavigationBarY , width: Int(UIScreen.main.bounds.size.width), height: 64))
